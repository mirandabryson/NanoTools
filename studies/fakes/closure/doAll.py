...
def doAll(start=0, stop=None, monolep=False, dilep=False):
    if monolep:
        print("Monolepton ttbar looper selected.")
        # Load .so files
        print("Loading cpp...")
        ...
        if stop:
            ...
            print("Parsing samples starting from {0}".format(start))
        else:
        ...
        for i, sample in enumerate(samples):
            if i < start:
                ...
                print(sample)
                ROOT.MonolepChain(mc, "output/ttbar-monolep-tautology_{}.root".format(i))
    elif dilep:
        print("Dilepton ttbar looper selected.")
        # Load .so files
        print("Loading cpp...")
        ROOT.gROOT.ProcessLine(".L ./NanoCORE/NANO_CORE.so");
        ROOT.gROOT.ProcessLine(".L ./dilep.C+");
        ROOT.gROOT.ProcessLine(".L DilepChain.C+");
        if stop:
            print("Parsing samples from {0} up until {1}".format(start, stop))
        elif start > 0:
            print("Parsing samples starting from {0}".format(start))
        ...
        for i, sample in enumerate(samples):
            if i < start:
                ...
                print("Running ScanChain over the following file:")
                print(sample)
                ROOT.DilepChain(mc, "output/ttbar-dilep_{}.root".format(i))
    else:
        print("No looper selected.")
...
if __name__ == "__main__":
    ...
    doAll(start=args.start, stop=args.stop, monolep=args.monolep, 
          dilep=args.dilep)
